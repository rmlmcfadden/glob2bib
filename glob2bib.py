...
def get_keys(aux_file, biblatex=False):
   ...
   revtex = [revtex41_citation, apsrev41_citation, revtex42_citation, apsrev42_citation]
   ...
   with open(aux_file, 'r') as fh:
      ...
      for line in fh.readlines():
         ...
         # BibLaTeX backend
         if biblatex == True and biblatex_citation in line and line != biblatex_empty_citation:
...
def replace_greek(text):
   ...
   greek_letter = ['α', 'β', 'γ', 'Γ', 'δ', 'Δ', 'ϵ', 'ζ', 'η', 'θ', 'Θ', 'ι', 'κ', 'λ', 'Λ', 'μ', 'ν', 'π', 'Π', 'ρ', 'σ', 'Σ', 'τ', 'υ', 'Υ', 'ϕ', 'Φ', 'χ', 'ψ', 'Ψ', 'ω', 'Ω']
   english_name = ['alpha', 'beta', 'gamma', 'Gamma', 'delta', 'Delta', 'epsilon', 'zeta', 'eta', 'theta', 'Theta', 'iota', 'kappa', 'lambda', 'Lambda', 'mu', 'nu', 'pi', 'Pi', 'rho', 'sigma', 'Sigma', 'tau', 'upsilon', 'Upsilon', 'phi', 'Phi', 'chi', 'psi', 'Psi', 'omega', 'Omega']
...
if __name__ == "__main__":
   ...
   parser = ArgumentParser(prog='glob2bib', description='glob2bib: a bibliography generator for BibTeX and BibLaTeX/Biber', epilog='Copyright (c) 2019 Ryan M. L. McFadden',)
   
   # positional arguments
   parser.add_argument('aux_file', help='auxillary file (.aux) from LaTeX compilation', type=str, )
   parser.add_argument('bib_dir', help='directory to recursively glob for *.bib files', type=str, )
   
   # optional arguments
   parser.add_argument('-v', '--version', action='version', version='%(prog)s v0.1',)
   parser.add_argument('-o', '--output', help='name of output file', type=str, )
   parser.add_argument('-s', '--substitute-unicode', default=False, action='store_true', help='substitute unicode greek literals to (escaped) greek math TeX commands')
   parser.add_argument('-b', '--biblatex', default=False, action='store_true', help='assume BibLaTeX/Biber as the bibliography management backend')
